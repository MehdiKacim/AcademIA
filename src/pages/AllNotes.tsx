...
const AllNotes = () => {
  ...
  useEffect(() => {
    ...
    setAllNotes(notes);
    // Select the first note group by default if available
  ...
  return (
    <div className="space-y-8 h-[calc(100vh-120px)] flex flex-col"> {/* Adjusted height for layout */}
      <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary via-foreground to-primary bg-[length:200%_auto] animate-background-pan">
        Toutes mes notes
      </h1>
      <p className="text-lg text-muted-foreground">
        Retrouvez ici toutes les notes que vous avez prises pour vos cours et modules. Sélectionnez une carte pour voir et gérer les notes.
      </p>
                  ...
                  <div className="grid gap-4">
                    {filteredNotes.map((noteGroup) => (
                      <Card
                        ...
                        onClick={() => setSelectedNoteGroupKey(noteGroup.key)}
                      >
                        <CardHeader>
