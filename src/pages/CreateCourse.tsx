...
import { Course, Module, ModuleSection, QuizQuestion, QuizOption } from "@/lib/dataModels"; // Import interfaces from dataModels
...
const CourseSchema = z.object({
  ...
  image_url: z.string().url("L'URL de l'image du cours doit être valide.").optional().or(z.literal("")), // Changed to image_url
  skills_to_acquire: z.string().min(1, "Les compétences à acquérir sont requises (séparées par des virgules)."), // Changed to skills_to_acquire
...
const CreateCourse = () => {
  ...
  const onSubmit = async (values: z.infer<typeof CourseSchema>) => {
    const courseData: Course = {
      id: courseId || `course${Date.now()}`, // ID will be generated by Supabase, but keep for type consistency
      title: values.title,
      ...
      creator_id: 'dummy-creator-id', // Add a dummy creator_id for now, or fetch from currentUserProfile.id
    };
