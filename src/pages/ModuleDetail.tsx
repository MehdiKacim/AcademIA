...
interface Course {
  ...
  description: string;
  modules: Module[];
  skillsToAcquire: string[]; // Ajouté pour la cohérence
}
...
const initialDummyCourses: Course[] = [
  {
    ...
    modules: [
      ...
      { title: "Module 2: Apprentissage Automatique", content: "Concepts de base du Machine Learning, supervisé et non supervisé. Le Machine Learning est une branche de l'IA qui permet aux systèmes d'apprendre à partir de données sans être explicitement programmés. L'apprentissage supervisé utilise des données étiquetées pour entraîner des modèles (ex: classification, régression), tandis que l'apprentissage non supervisé trouve des motifs dans des données non étiquetées (ex: clustering, réduction de dimensionnalité).", isCompleted: false },
      { title: "Module 3: Réseaux de Neurones", content: "Introduction aux réseaux de neurones et au Deep Learning. Les réseaux de neurones sont des modèles inspirés du cerveau humain, composés de couches de 'neurones' interconnectés. Le Deep Learning, une sous-catégorie du Machine Learning, utilise des réseaux de neurones profonds (avec de nombreuses couches) pour apprendre des représentations complexes des données, ce qui a révolutionné des domaines comme la vision par ordinateur et le traitement du langage naturel.", isCompleted: false },
      { title: "Module 4: Applications de l'IA", content: "Exemples concrets : vision par ordinateur, traitement du langage naturel. L'IA est omniprésente : reconnaissance faciale, assistants vocaux, voitures autonomes, systèmes de recommandation, diagnostics médicaux. La vision par ordinateur permet aux machines de 'voir' et d'interpréter des images, tandis que le traitement du langage naturel (NLP) leur permet de comprendre et de générer du langage humain.", isCompleted: false },
    ],
