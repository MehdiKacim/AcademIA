  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn(
        ...
        "relative" // Add relative positioning for the absolute close button
      )}>
        {/* The "puce" (close button) */}
        <Button
          variant="ghost"
          size="icon"
          onClick={onClose}
          className="absolute top-4 right-4 z-10 rounded-full bg-muted/20 hover:bg-muted/40"
          aria-label="Fermer"
        >
          <X className="h-5 w-5" />
        </Button>
