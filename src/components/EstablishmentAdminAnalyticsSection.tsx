...
interface EstablishmentAdminAnalyticsSectionProps {
  establishments: Establishment[];
  curricula: Curriculum[];
  classes: Class[];
  allProfiles: Profile[];
  selectedEstablishmentFilter: string; // This prop is now mandatory for this component
}

const EstablishmentAdminAnalyticsSection = ({ establishments, curricula, classes, allProfiles, selectedEstablishmentFilter }: EstablishmentAdminAnalyticsSectionProps) => {
  ...
  const filteredEstablishments = establishments.filter(est => est.id === selectedEstablishmentFilter);

  const filteredCurricula = curricula.filter(cur => cur.establishment_id === selectedEstablishmentFilter);

  const filteredClasses = classes.filter(cls => cls.establishment_id === selectedEstablishmentFilter);

  const filteredProfiles = allProfiles.filter(p => p.establishment_id === selectedEstablishmentFilter);
  ...
  const currentEstablishmentName = establishments.find(est => est.id === selectedEstablishmentFilter)?.name || 'Établissement Inconnu';
  ...
  return (
    <>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setIsEstablishmentDetailModalOpen(true)}>
          <CardHeader className="flex flex-row items-center gap-3 pb-3">
            ...
            <CardTitle className="text-lg">Détail de l'Établissement</CardTitle>
          </CardHeader>
          <CardContent>
            <CardDescription>Voir toutes les statistiques détaillées de l'établissement.</CardDescription>
          </CardContent>
        </Card>
      </div>
      ...
      <UserListModal
        ...
        description="Tous les directeurs de cet établissement."
      ...
      <UserListModal
        ...
        description="Tous les directeurs adjoints de cet établissement."
      ...
      <UserListModal
        ...
        description="Tous les élèves de cet établissement."
      ...
      <UserListModal
        ...
        description="Tous les professeurs de cet établissement."
      ...
      <UserListModal
        ...
        description="Tous les tuteurs de cet établissement."
      ...
      <CurriculumListModal
        ...
        description="Tous les cursus de cet établissement."
        curricula={filteredCurricula}
        establishments={establishments} // Use all establishments for name resolution
      />
      <ClassListModal
        ...
        description="Toutes les classes de cet établissement."
        classes={filteredClasses}
...
export default EstablishmentAdminAnalyticsSection;